...
public class PerkController {
        ...
        try {
            ...
            if (currentUser != null) {
                ...
                for (Perk p : pageItems) {
                    ...
                    else if (p.getDownvotedBy().stream().anyMatch(u -> u.getId().equals(currentUser.getId())))
                        voteState = -1;
    ...
    public String addPerk(@RequestParam(required = false) Long productId,
        ...
        } else if (benefitTrim.length() > Perk.BENEFIT_MAX_LENGTH) {
            fieldErrors.put("benefit", "Benefit must be at most " + Perk.BENEFIT_MAX_LENGTH + " characters");
        }

        if (regionTrim != null && !regionTrim.isEmpty() && regionTrim.length() > Perk.REGION_MAX_LENGTH) {
            fieldErrors.put("region", "Region must be at most " + Perk.REGION_MAX_LENGTH + " characters");
        }
